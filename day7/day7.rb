...
def can_contain_gold?(bag_color)
  ...
  $already_known[bag_color] = $rules[bag_color].map { |r| can_contain_gold?(r[0]) }.any? # recurse if not found
  $already_known[bag_color]
end

total = 0
$rules.each do |color, _contains|
  total += 1 if can_contain_gold?(color)
end
...
def bag_contains(bag_color)
  ...
  $rules[bag_color].each do |bag, count|
    total += count.to_i * bag_contains(bag)
  end
  total
end
